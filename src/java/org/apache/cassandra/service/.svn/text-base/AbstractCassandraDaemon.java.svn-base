...
import imc.jettyserver.DisXMLDbServer;
...
public abstract class AbstractCassandraDaemon implements CassandraDaemon
{
    ...
    protected DisXMLDbServer xmlServer  = new DisXMLDbServer();
    /**
    ...
    public void activate()
    {
        ...
        try
        {
            ...
            try {
            	XMLDBStore.instance().init();
      			xmlServer.run(new String[]{DatabaseDescriptor.getXmlRpcAddress().getHostAddress(), Integer.toString(DatabaseDescriptor.getXmlRpcPort())});
      		} catch (Exception e1) {
