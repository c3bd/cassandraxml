...
/**
 * this file is modified by xiafan(xiafan68@gmail.com)
 * @time 2011.10.09
 */
...
public class StorageProxy implements StorageProxyMBean {
	...
	private static final LatencyTracker localWriteStats = new LatencyTracker();

	private static final WriteResponse FAKE_WRITE_RESPONSE = new WriteResponse(
			SysConfig.DEFAULT_KEYSPACE, ByteBufferUtil.EMPTY_BYTE_BUFFER,
			false);
	...
	private static volatile boolean hintedHandoffEnabled = DatabaseDescriptor
			.hintedHandoffEnabled();
	private static volatile int maxHintWindow = DatabaseDescriptor
			.getMaxHintWindow();
	private static volatile int maxHintsInProgress = 1024 * Runtime
	...
	static {
		...
		/*
		 * We execute counter writes in 2 places: either directly in the
		 * coordinator node if it is a replica, or in CounterMutationVerbHandler
		 * on a replica othewise. The write must be executed on the MUTATION
		 * stage but on the latter case, the verb handler already run on the
		 * MUTATION stage, so we must not execute the underlying on the stage
		 * otherwise we risk a deadlock. Hence two different performer.
		 */
	...
	/**
	 * Use this method to have these Mutations applied across all replicas. This
	 * method will take care of the possibility of a replica being down and hint
		...
		try {
			for (IMutation mutation : mutations) {
				...
				if (mutation instanceof CounterMutation) {
					responseHandlers.add(mutateCounter(
							(CounterMutation) mutation, localDataCenter));
				} else {
					responseHandlers.add(performWrite(mutation,
							consistency_level, localDataCenter,
			...
			for (IWriteResponseHandler responseHandler : responseHandlers) {
				responseHandler.get();
				if (responseHandler.getExMessage() != null)
					throw new TimeoutException(responseHandler.getExMessage());
			}
	...
	public static void mutate(List<? extends IMutation> mutations,
			ConsistencyLevel consistency_level, List<Integer> modifyCounts)
		...
		try {
			...
			int i = 0;
			for (IWriteResponseHandler responseHandler : responseHandlers) {
				modifyCounts.set(i++, responseHandler.get());
				if (responseHandler.getExMessage() != null)
					throw new TimeoutException(responseHandler.getExMessage());
			}
	...
	/**
	 * Perform the write of a mutation given a WritePerformer. Gather the list
	 * of write endpoints, apply locally and/or forward the mutation to said
	 * write endpoint (deletaged to the actual WritePerformer) and wait for the
	 ...
	 * @param performer
	 *            the WritePerformer in charge of appliying the mutation given
	 *            the list of write endpoints (either standardWritePerformer for
	...
	public static IWriteResponseHandler performWrite(IMutation mutation,
			...
			WritePerformer performer) throws UnavailableException,
			TimeoutException, IOException {
		String table = mutation.getTable();
		AbstractReplicationStrategy rs = Table.open(table)
				.getReplicationStrategy();

		Collection<InetAddress> writeEndpoints = getWriteEndpoints(table,
				mutation.key());

		IWriteResponseHandler responseHandler = rs.getWriteResponseHandler(
				writeEndpoints, consistency_level);

		// exit early if we can't fulfill the CL at this time
		responseHandler.assureSufficientLiveNodes();

		performer.apply(mutation, writeEndpoints, responseHandler,
	...
	private static Collection<InetAddress> getWriteEndpoints(String table,
		...
		return ss.getTokenMetadata().getWriteEndpoints(
				StorageService.getPartitioner().getToken(key), table,
				naturalEndpoints);
	}

	/**
	 * Send the mutations to the right targets, write it locally if it
	 * corresponds or writes a hint when the node is not available.
	 * 
	 * Note about hints:
	 * 
	 * | Hinted Handoff | Consist. Level | | on | >=1 | --> wait for hints. We
	 * DO NOT notify the handler with handler.response() for hints; | on | ANY |
	 * --> wait for hints. Responses count towards consistency. | off | >=1 |
	 * --> DO NOT fire hints. And DO NOT wait for them to complete. | off | ANY
	 * | --> DO NOT fire hints. And DO NOT wait for them to complete.
	 * 
		...
		for (InetAddress destination : targets) {
			if (FailureDetector.instance.isAlive(destination)) {
				...
				if (destination.equals(FBUtilities.getBroadcastAddress())) {
					...
					Multimap<Message, InetAddress> messages = dcMessages
							.get(dc);
					if (messages == null) {
					...
					messages.put(producer.getMessage(Gossiper.instance
							.getVersion(destination)), destination);
				}
			} else {
				if (!shouldHint(destination))
					continue;

				// Avoid OOMing from hints waiting to be written. (Unlike
				// ordinary mutations, hint
				// not eligible to drop if we fall behind.)
				if (hintsInProgress.get() > maxHintsInProgress)
					throw new TimeoutException();

				// Schedule a local hint and let the handler know it needs to
				// wait for the hint to complete too
				Future<Void> hintfuture = scheduleLocalHint(rm, destination,
						responseHandler, consistency_level);
				responseHandler
						.addFutureForHint(new CreationTimeAwareFuture<Void>(
								hintfuture));
			}
		}
	...
	public static Future<Void> scheduleLocalHint(final RowMutation mutation,
			...
			final ConsistencyLevel consistencyLevel) throws IOException {
		...
		Runnable runnable = new Runnable() {
			...
			public void run() {
				...
				try {
					Token<?> token = StorageService.instance.getTokenMetadata()
							.getToken(target);
					ByteBuffer tokenbytes = StorageService.getPartitioner()
							.getTokenFactory().toByteArray(token);
					...
					if (responseHandler != null
						...
						responseHandler.response(FAKE_WRITE_RESPONSE);
				} catch (IOException e) {
					throw new RuntimeException(e);
				} finally {
					// Decrement the current hint in the execution after the
					// task is done.
		...
		return (Future<Void>) StageManager.getStage(Stage.MUTATION).submit(
				runnable);
	}
	 ...
	 * other replicas
	 */
	private static void sendMessages(String localDataCenter,
			Map<String, Multimap<Message, InetAddress>> dcMessages,
			IWriteResponseHandler handler) throws IOException {
		for (Map.Entry<String, Multimap<Message, InetAddress>> entry : dcMessages
				.entrySet()) {
			String dataCenter = entry.getKey();

			// send the messages corresponding to this datacenter
			for (Map.Entry<Message, Collection<InetAddress>> messages : entry
					.getValue().asMap().entrySet()) {
				Message message = messages.getKey();
				// a single message object is used for unhinted writes, so clean
				// out any forwards
				...
				if (dataCenter.equals(localDataCenter)
					...
					while (iter.hasNext()) {
						...
						byte[] previousHints = message
								.getHeader(RowMutation.FORWARD_HEADER);
					...
					MessagingService.instance()
							.sendRR(message, target, handler);
				}
			}
		}
	}

	private static void insertLocal(final RowMutation rm,
			final IWriteResponseHandler responseHandler) {
		if (logger.isDebugEnabled())
			logger.debug("insert writing local " + rm.toString(true));
		Runnable runnable = new DroppableRunnable(StorageService.Verb.MUTATION) {
			public void runMayThrow() throws XMLDBException, IOException {
				long start = System.currentTimeMillis();
				RowMutation cp = rm;
				
				int modifyCount = 0;
				if (cp.getCommand() == XUpdateCommand.update) {
					CollectionStore colStore = XMLDBStore.instance()
							.getCollection(cp.getColID());
					//cp = colStore.modifyUpdateMutation(cp);
					cp.apply();
				} else if (cp.getCommand() == XUpdateCommand.delete) {
					CollectionStore colStore = XMLDBStore.instance()
							.getCollection(cp.getColID());
					// delete all the child nodes
					CFMetaData meta = colStore.getCfStore().getCfStore().metadata;
					int xmlID = Integer.parseInt(meta.getKeyValidator()
							.getString(cp.key()));
					for (ColumnFamily cf : cp.getColumnFamilies()) {
						for (IColumn col : cf.getSortedColumns()) {
							int nodeID = Integer.parseInt(meta
									.getComparatorFor(null).getString(
											col.name()));
				...
				/*
				 * if (rm.getColID() != -1) {
				 * CollectionStore.storeXMLNodeLocal(rm); }
				 */
				WriteResponse response = new WriteResponse(rm.getTable(),
						rm.key(), true);
				response.modifyCount = modifyCount;
				responseHandler.response(response);
				localWriteStats.addMicro(System.currentTimeMillis() - start);
			}
		};
		StageManager.getStage(Stage.MUTATION).execute(runnable);
	}
	 ...
	 * call the leader for the mutation) before being replicated to the other
	 * endpoint. To achieve so, there is two case: 1) the coordinator host is a
	 * replica: we proceed to applying the update locally and replicate throug
	 * applyCounterMutationOnCoordinator 2) the coordinator is not a replica: we
	 * forward the (counter)mutation to a chosen replica (that will proceed
	 * through applyCounterMutationOnLeader upon receive) and wait for its
	 * acknowledgment.
	 * 
	 * Implementation note: We check if we can fulfill the CL on the coordinator
	 * host even if he is not a replica to allow quicker response and because
	 * the WriteResponseHandlers don't make it easy to send back an error. We
	 * also always gather the write latencies at the coordinator node to make
	 * gathering point similar to the case of standard writes.
	 */
	public static IWriteResponseHandler mutateCounter(CounterMutation cm,
			String localDataCenter) throws UnavailableException,
			TimeoutException, IOException {
		...
		if (endpoint.equals(FBUtilities.getBroadcastAddress())) {
			...
			rs.getWriteResponseHandler(writeEndpoints, cm.consistency())
					.assureSufficientLiveNodes();

			// Forward the actual update to the chosen leader replica
			IWriteResponseHandler responseHandler = WriteResponseHandler
					.create(endpoint);

			Message message = cm.makeMutationMessage(Gossiper.instance
					.getVersion(endpoint));
			if (logger.isDebugEnabled())
				logger.debug("forwarding counter update of key "
						+ ByteBufferUtil.bytesToHex(cm.key()) + " to "
						+ endpoint);
	...
	/**
	 * Find a suitable replica as leader for counter update. For now, we pick a
	 * random replica in the local DC (or ask the snitch if there is no replica
	 * alive in the local DC). TODO: if we track the latency of the counter
	 * writes (which makes sense contrarily to standard writes since there is a
	 * read involved), we could trust the dynamic snitch entirely, which may be
	 * a better solution. It is unclear we want to mix those latencies with read
	 * latencies, so this may be a bit involved.
	 */
	private static InetAddress findSuitableEndpoint(String table,
		...
		List<InetAddress> endpoints = StorageService.instance
				.getLiveNaturalEndpoints(table, key);
		...
		if (localEndpoints.isEmpty()) {
			...
			return localEndpoints.get(FBUtilities.threadLocalRandom().nextInt(
					localEndpoints.size()));
		}
	}
	...
	public static IWriteResponseHandler applyCounterMutationOnLeader(
			CounterMutation cm, String localDataCenter)
	...
	// Same as applyCounterMutationOnLeader but must with the difference that it
	// use the MUTATION stage to execute the write (while
	// applyCounterMutationOnLeader assumes it is on the MUTATION stage already)
	public static IWriteResponseHandler applyCounterMutationOnCoordinator(
			CounterMutation cm, String localDataCenter)
			throws UnavailableException, TimeoutException, IOException {
		return performWrite(cm, cm.consistency(), localDataCenter,
		...
		return new DroppableRunnable(StorageService.Verb.MUTATION) {
			public void runMayThrow() throws IOException {
				...
				if (cm.shouldReplicateOnWrite() && !targets.isEmpty()) {
					...
					StageManager.getStage(Stage.REPLICATE_ON_WRITE).execute(
							new DroppableRunnable(StorageService.Verb.READ) {
								public void runMayThrow() throws IOException,
										TimeoutException {
									// send mutation to other replica
									sendToHintedEndpoints(
											cm.makeReplicationMutation(),
											targets, responseHandler,
											localDataCenter, consistency_level);
								}
							});
				}
	 ...
	 * snitch 2. Send a data request to the closest replica, and digest requests
	 * to either a) all the replicas, if read repair is enabled b) the closest
	 * R-1 replicas, where R is the number required to satisfy the
	 * ConsistencyLevel 3. Wait for a response from R replicas 4. If the digests
	 * (if any) match the data return the data 5. else carry out read repair by
	 * getting data from all the nodes.
	 */
	private static List<Row> fetchRows(List<ReadCommand> initialCommands,
		...
		do {
			...
			for (int i = 0; i < commands.size(); i++) {
				...
				RowDigestResolver resolver = new RowDigestResolver(
				...
				if (dataPoint.equals(FBUtilities.getBroadcastAddress())) {
					...
					StageManager.getStage(Stage.READ).execute(
							new LocalReadRunnable(command, handler));
				} else {
				...
				for (InetAddress digestPoint : handler.endpoints.subList(1,
					...
					if (digestPoint.equals(FBUtilities.getBroadcastAddress())) {
						...
						StageManager.getStage(Stage.READ).execute(
								new LocalReadRunnable(digestCommand, handler));
					} else {
			...
			for (int i = 0; i < commands.size(); i++) {
				...
				try {
					...
					RowRepairResolver resolver = new RowRepairResolver(
							command.table, command.key);
					RepairCallback repairHandler = new RepairCallback(resolver,
							handler.endpoints);
					...
					MessageProducer producer = new CachingMessageProducer(
							command);
			...
			if (repairResponseHandlers != null) {
				for (int i = 0; i < repairCommands.size(); i++) {
					...
					if (command instanceof SliceFromReadCommand) {
						...
						int liveColumnsInRow = row != null ? row.cf
								.getLiveColumnCount() : 0;

						assert maxLiveColumns <= sliceCommand.count;
						if ((maxLiveColumns == sliceCommand.count)
								&& (liveColumnsInRow < sliceCommand.count)) {
							logger.debug(
									"detected short read: expected {} columns, but only resolved {} columns",
									sliceCommand.count, liveColumnsInRow);

							int retryCount = sliceCommand.count
									+ sliceCommand.count - liveColumnsInRow;
							SliceFromReadCommand retryCommand = new SliceFromReadCommand(
									command.table, command.key,
									command.queryPath, sliceCommand.start,
									sliceCommand.finish, sliceCommand.reversed,
									retryCount);
							if (commandsToRetry == Collections.EMPTY_LIST)
	...
	/*
	 * This function executes the read protocol locally. Consistency checks are
	...
	public static List<Row> getRangeSlice(RangeSliceCommand command,
		...
		try {
			...
			for (AbstractBounds range : ranges) {
				List<InetAddress> liveEndpoints = StorageService.instance
						.getLiveNaturalEndpoints(command.keyspace, range.right);
				DatabaseDescriptor.getEndpointSnitch().sortByProximity(
						FBUtilities.getBroadcastAddress(), liveEndpoints);

				if (consistency_level == ConsistencyLevel.ONE
						&& !liveEndpoints.isEmpty()
						&& liveEndpoints.get(0).equals(
								FBUtilities.getBroadcastAddress())) {
					if (logger.isDebugEnabled())
						logger.debug("local range slice");
					ColumnFamilyStore cfs = Table.open(command.keyspace)
							.getColumnFamilyStore(command.column_family);
					try {
						rows.addAll(cfs.getRangeSlice(
					...
					RangeSliceCommand c2 = new RangeSliceCommand(
							command.keyspace, command.column_family,
							command.super_column, command.predicate, range,
							command.max_keys);

					// collect replies and resolve according to consistency
					// level
					RangeSliceResponseResolver resolver = new RangeSliceResponseResolver(
							command.keyspace, liveEndpoints);
					ReadCallback<Iterable<Row>> handler = getReadCallback(
							resolver, command, consistency_level, liveEndpoints);
					handler.assureSufficientLiveNodes();
	...
	/**
	 * initiate a request/response session with each live node to check whether
	 * or not everybody is using the same migration id. This is useful for
	 * determining if a schema change has propagated through the cluster.
	 * Disagreement is assumed if any node fails to respond.
	 */
	public static Map<String, List<String>> describeSchemaVersions() {
		...
		IAsyncCallback cb = new IAsyncCallback() {
			public void response(Message message) {
				...
				UUID theirVersion = UUID.fromString(new String(message
						.getMessageBody()));
		...
		try {
			...
			throw new AssertionError(
					"This latch shouldn't have been interrupted.");
		}
		...
		Iterable<InetAddress> allHosts = Iterables.concat(
				Gossiper.instance.getLiveMembers(),
				Gossiper.instance.getUnreachableMembers());
		for (InetAddress host : allHosts) {
			UUID version = versions.get(host);
			String stringVersion = version == null ? UNREACHABLE : version
					.toString();
	 ...
	 * replica destinations for many ranges, so we need to restrict each scan to
	 * the specific range we want, or else we'd get duplicate results.
	 */
	public static List<AbstractBounds> getRestrictedRanges(
			final AbstractBounds queryRange) {
		// special case for bounds containing exactly 1 (non-minimum) token
		if (queryRange instanceof Bounds
				&& queryRange.left.equals(queryRange.right)
				&& !queryRange.left.equals(StorageService.getPartitioner()
						.getMinimumToken())) {
			if (logger.isDebugEnabled())
		...
		TokenMetadata tokenMetadata = StorageService.instance
				.getTokenMetadata();

		List<AbstractBounds> ranges = new ArrayList<AbstractBounds>();
		// divide the queryRange into pieces delimited by the ring and minimum
		// tokens
		Iterator<Token> ringIter = TokenMetadata.ringIterator(
				tokenMetadata.sortedTokens(), queryRange.left, true);
		AbstractBounds remainder = queryRange;
		while (ringIter.hasNext()) {
			Token token = ringIter.next();
			if (remainder == null
					|| !(remainder.left.equals(token) || remainder
							.contains(token)))
				// no more splits
				break;
			Pair<AbstractBounds, AbstractBounds> splits = remainder
					.split(token);
			if (splits.left != null)
		...
		if (logger.isDebugEnabled())
			logger.debug("restricted ranges for query " + queryRange + " are "
					+ ranges);
	...
	public static List<Row> scan(final String keyspace, String column_family,
		...
		for (AbstractBounds range : ranges) {
			List<InetAddress> liveEndpoints = StorageService.instance
					.getLiveNaturalEndpoints(keyspace, range.right);
			DatabaseDescriptor.getEndpointSnitch().sortByProximity(
					FBUtilities.getBroadcastAddress(), liveEndpoints);

			// collect replies and resolve according to consistency level
			RangeSliceResponseResolver resolver = new RangeSliceResponseResolver(
					keyspace, liveEndpoints);
	...
	public static void truncateBlocking(String keyspace, String cfname)
			throws UnavailableException, TimeoutException, IOException {
		logger.debug(
				"Starting a blocking truncate operation on keyspace {}, CF ",
				keyspace, cfname);
		if (isAnyHostDown()) {
			logger.info("Cannot perform truncate, some hosts are down");
			// Since the truncate operation is so aggressive and is typically
			// only
			// invoked by an admin, for simplicity we require that all nodes are
			// up
		...
		final TruncateResponseHandler responseHandler = new TruncateResponseHandler(
				blockFor);
		...
		// Wait for all
		logger.debug(
				"Sent all truncate messages, now waiting for {} responses",
				blockFor);
		responseHandler.get();
	...
	@Override
	public long getWriteLocalMicros() {
		return localWriteStats.getTotalLatencyMicros();
	}

	@Override
	public double getRecentWriteLocalMicros() {
		return localWriteStats.getRecentLatencyMicros();
	}

	@Override
	public long[] getRecentWriteLocalHistogramMicros() {
		return localWriteStats.getRecentLatencyHistogramMicros();
	}
}
