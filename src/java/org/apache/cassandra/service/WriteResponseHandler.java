...
/**
 * Handles blocking writes for ONE, ANY, TWO, THREE, QUORUM, and ALL consistency
 * levels.
 */
public class WriteResponseHandler extends AbstractWriteResponseHandler {
	protected static final Logger logger = LoggerFactory
			.getLogger(WriteResponseHandler.class);
	...
	public void response(Message m) {
		ByteArrayInputStream bis = new ByteArrayInputStream(m.getMessageBody());
		try {
			WriteResponse response = WriteResponse.serializer().deserialize(
					new DataInputStream(bis), m.getVersion());
			if (response.isSuccess())
				modifyCount = response.modifyCount;
			else
				exMessage = response.exMessage;
		} catch (IOException e) {
			logger.error("response message {" + m.toString() + "} error : "
					+ e.getMessage());
		}
	...
	public void assureSufficientLiveNodes() throws UnavailableException {
		if (consistencyLevel == ConsistencyLevel.ANY) {
			// Ensure there are blockFor distinct living nodes (hints (local)
			// are ok).
			// Thus we include the local node (coordinator) as a valid replica
			// if it is there already.
			int effectiveEndpoints = writeEndpoints.contains(FBUtilities
					.getBroadcastAddress()) ? writeEndpoints.size()
					: writeEndpoints.size() + 1;
