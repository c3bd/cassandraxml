...
public class SuperColumn extends AbstractColumnContainer implements IColumn {
	...
	public int serializedSize() {
		 ...
		 * with the way we are calculating the size for the column family
		 * serializer.
		 */
	...
	// 该函数只能用于sp对应于xml节点的情况
	public long timestamp() {
		/**
		 * 从节点的删除时间，tagName觉得的节点创建时间以及text决定的文本更新时间决定那个东西最新
		 */
		long retTime = 0;
		if (getMarkedForDeleteAt() != Long.MIN_VALUE)
			retTime = getMarkedForDeleteAt();
		if (getColumn(CollectionStore.TAGNAME) != null
				&& getColumn(CollectionStore.TAGNAME).timestamp() > retTime)
			retTime = getColumn(CollectionStore.TAGNAME).timestamp();
		if (getColumn(CollectionStore.VALUE) != null
				&& getColumn(CollectionStore.VALUE).timestamp() > retTime)
			retTime = getColumn(CollectionStore.VALUE).timestamp();

		return retTime;
		// throw new
		// UnsupportedOperationException("This operation is not supported for Super Columns.");
	}
	...
	/*
	 * Go through each sub column if it exists then as it to resolve itself if
	 * the column does not exist then create it.
	 */
	...
	public IColumn diff(IColumn columnNew) {
		...
		if (columnNew.getMarkedForDeleteAt() > getMarkedForDeleteAt()) {
			((SuperColumn) columnDiff).delete(columnNew.getLocalDeletionTime(),
	...
	public String getString(AbstractType comparator) {
		...
		if (isMarkedForDelete()) {
			sb.append(" -delete at ").append(getMarkedForDeleteAt())
					.append("-");
		}
	...
	public IColumn localCopy(ColumnFamilyStore cfs, Allocator allocator) {
		...
		// dynamic values
		SuperColumn sc = new SuperColumn(allocator.clone(name),
				this.getComparator());
		// since deletion info is immutable, aliasing it is fine
		sc.deletionInfo.set(deletionInfo.get());
	...
	public IColumn reconcile(IColumn c) {
		/*
		 * 之所以用c去reconcile本column,是应为xupdateText的时候只会存入修改了的列，其它情况下都是整体更新
		 */
		if (this.getMarkedForDeleteAt() > c.getMarkedForDeleteAt())
			return this;
		else if (this.getMarkedForDeleteAt() < c.getMarkedForDeleteAt())
			return c;
		for (IColumn oColumn : c.getSubColumns()) {
			IColumn iColumn = getSubColumn(oColumn.name());
			if (iColumn == null)
				return c;
			if (iColumn.reconcile(oColumn) != oColumn)
				return this;
		}
		return c;
	}
	...
	public IColumn reconcile(IColumn c, Allocator allocator) {
		throw new UnsupportedOperationException(
				"This operation is unsupported on super columns.");
	}

	public int serializationFlags() {
		throw new UnsupportedOperationException(
				"Super columns don't have a serialization mask");
	}
...
class SuperColumnSerializer implements IColumnSerializer {
	...
	public IColumn deserialize(DataInput dis, boolean fromRemote)
			throws IOException {
		return deserialize(dis, fromRemote,
				(int) (System.currentTimeMillis() / 1000));
	}

	public IColumn deserialize(DataInput dis, boolean fromRemote,
			int expireBefore) throws IOException {
		...
		ColumnSortedMap preSortedMap = new ColumnSortedMap(comparator,
				serializer, dis, size, fromRemote, expireBefore);
		SuperColumn superColumn = new SuperColumn(name, ThreadSafeSortedColumns
				.factory().fromSorted(preSortedMap, false));
		if (localDeleteTime != Integer.MIN_VALUE && localDeleteTime <= 0) {
			throw new IOException("Invalid localDeleteTime read: "
					+ localDeleteTime);
		}
