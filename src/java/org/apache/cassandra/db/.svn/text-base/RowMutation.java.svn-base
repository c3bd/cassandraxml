...
/**
 * this file is changed by xiafan68(xiafan68@gmail.com)
 */
...
public class RowMutation implements IMutation, MessageProducer {
	...
	public enum XUpdateCommand {
		append, delete, update, store, other
	};

	private XUpdateCommand command = XUpdateCommand.other;
	private static final XUpdateCommand[] XUpdateCommands = new XUpdateCommand[] {
			XUpdateCommand.append, XUpdateCommand.delete,
			XUpdateCommand.update, XUpdateCommand.store, XUpdateCommand.other };
	...
	protected RowMutation(String table, ByteBuffer key,
	 ...
	 * as soon as it becomes available. The format is the following:
	 * 
	 * HintsColumnFamily: { // cf <dest ip>: { // key <uuid>: { // super-column
	 * table:
	 * <table>
	 * // columns key: <key> mutation: <mutation> version: <version> } } }
	 * 
	 */
	public static RowMutation hintFor(RowMutation mutation, ByteBuffer token)
			throws IOException {
		...
		// this is set at the smallest GCGraceSeconds for any of the CFs in the
		// RM
		...
		QueryPath path = new QueryPath(HintedHandOffManager.HINTS_CF, hintId,
				ByteBufferUtil.bytes("mutation"));
		rm.add(path, ByteBuffer.wrap(mutation
				.getSerializedBuffer(MessagingService.version_)), System
				.currentTimeMillis(), ttl);

		// serialization version
		path = new QueryPath(HintedHandOffManager.HINTS_CF, hintId,
				ByteBufferUtil.bytes("version"));
		rm.add(path, ByteBufferUtil.bytes(MessagingService.version_),
				System.currentTimeMillis(), ttl);

		// table
		path = new QueryPath(HintedHandOffManager.HINTS_CF, hintId,
				ByteBufferUtil.bytes("table"));
		rm.add(path, ByteBufferUtil.bytes(mutation.getTable()),
				System.currentTimeMillis(), ttl);
	...
	/*
	 * Specify a column name and a corresponding value for the column. Column
	 * name is specified as <column family>:column. This will result in a
	 * ColumnFamily associated with <column family> as name and a Column with
	 * <column> as name. The column can be further broken up as super column
	 * name : columnname in case of super columns
	 * 
	 * param @ cf - column name as <column family>:<column> param @ value -
	 * value associated with the column param @ timestamp - timestamp associated
	 * with this data. param @ timeToLive - ttl for the column, 0 for standard
	 * (non expiring) columns
	 */
	...
	/**
	 * this method is modified to add the storexml attribute to the header
	 * 
	 * @param verb
	 * @param version
	 * @return
	 * @throws IOException
	 */
	public Message makeRowMutationMessage(StorageService.Verb verb, int version)
			throws IOException {
		Message ret = new Message(FBUtilities.getBroadcastAddress(), verb,
				getSerializedBuffer(version), version);
		return ret;
	}

	public synchronized byte[] getSerializedBuffer(int version)
			throws IOException {
	...
	public XUpdateCommand getCommand() {
		return command;
	}

	public void setCommand(XUpdateCommand command) {
	...
	public String toString(boolean shallow) {
		...
		buff.append(", key='").append(ByteBufferUtil.bytesToHex(key_))
				.append('\'');
	...
	public void addColumnOrSuperColumn(String cfName, ColumnOrSuperColumn cosc) {
		if (cosc.super_column != null) {
			for (org.apache.cassandra.thrift.Column column : cosc.super_column.columns) {
				add(new QueryPath(cfName, cosc.super_column.name, column.name),
						column.value, column.timestamp, column.ttl);
			}
		...
		if (del.predicate != null && del.predicate.column_names != null) {
			for (ByteBuffer c : del.predicate.column_names) {
				if (del.super_column == null
						&& Schema.instance.getColumnFamilyType(table_, cfName) == ColumnFamilyType.Super)
	...
	public static RowMutation fromBytes(byte[] raw, int version)
			throws IOException {
		RowMutation rm = serializer_.deserialize(new DataInputStream(
				new FastByteArrayInputStream(raw)), version);
		boolean hasCounters = false;
		for (Map.Entry<Integer, ColumnFamily> entry : rm.modifications_
				.entrySet()) {
			if (entry.getValue().metadata().getDefaultValidator()
					.isCommutative()) {
				hasCounters = true;
				break;
			}
	...
	public static class RowMutationSerializer implements
		...
		public void serialize(RowMutation rm, DataOutputStream dos, int version)
				throws IOException {
			dos.writeUTF(rm.getTable());
			ByteBufferUtil.writeWithShortLength(rm.key(), dos);
			dos.writeInt(rm.getColID());
			dos.writeInt(rm.command.ordinal());
		...
		public RowMutation deserialize(DataInputStream dis, int version,
				boolean fromRemote) throws IOException {
			String table = dis.readUTF();
			ByteBuffer key = ByteBufferUtil.readWithShortLength(dis);
			int colID = dis.readInt();
			XUpdateCommand command = XUpdateCommands[dis.readInt()];
			...
			RowMutation ret = new RowMutation(table, key, modifications);
			ret.setColID(colID);
