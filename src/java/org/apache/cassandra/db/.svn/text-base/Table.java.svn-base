...
public class Table implements TableMBean {
	...
	private static final LatencyTracker applyLatencyTracker = new LatencyTracker();
	private static final LatencyTracker readIdxColLatencyTracker = new LatencyTracker();
	private static final LatencyTracker updateIdxLatencyTracker = new LatencyTracker();
	private static final LatencyTracker cfsApplyLatencyTracker = new LatencyTracker();

	private static final LatencyTracker prepareApplyLatencyTracker = new LatencyTracker();
	private static final LatencyTracker applyPartLatencyTracker = new LatencyTracker();
	 ...
	 * 
	 * (Enabling fairness in the RRWL is observed to decrease throughput, so we
	 * leave it off.)
	 */
	...
	public static Table open(String table, Schema schema) {
		...
		if (tableInstance == null) {
			// instantiate the Table. we could use putIfAbsent but it's
			...
			synchronized (Table.class) {
				...
				if (tableInstance == null) {
					...
					for (ColumnFamilyStore cfs : tableInstance
	...
	public ColumnFamilyStore getColumnFamilyStore(String cfName) {
		...
		if (id == null)
			throw new IllegalArgumentException(String.format(
					"Unknown table/cf pair (%s.%s)", name, cfName));
	 ...
	 */
	public void forceCleanup(NodeId.OneShotRenewer renewer) throws IOException,
			ExecutionException, InterruptedException {
		if (name.equals(SYSTEM_TABLE))
			throw new UnsupportedOperationException(
					"Cleanup of the system table is neither necessary nor wise");

		// Sort the column families in order of SSTable size, so cleanup of
		// smaller CFs
		// can free up space for larger ones
		List<ColumnFamilyStore> sortedColumnFamilies = new ArrayList<ColumnFamilyStore>(
				columnFamilyStores.values());
		Collections.sort(sortedColumnFamilies,
				new Comparator<ColumnFamilyStore>() {
					// Compare first on size and, if equal, sort by name
					// (arbitrary & deterministic).
					public int compare(ColumnFamilyStore cf1,
							ColumnFamilyStore cf2) {
						long diff = (cf1.getTotalDiskSpaceUsed() - cf2
	...
	public boolean snapshotExists(String snapshotName) {
		for (String dataDirPath : DatabaseDescriptor.getAllDataFileLocations()) {
			String snapshotPath = dataDirPath + File.separator + name
					+ File.separator + SNAPSHOT_SUBDIR_NAME + File.separator
					+ snapshotName;
	...
	private Table(String table) {
		...
		for (CFMetaData cfm : new ArrayList<CFMetaData>(Schema.instance
				.getTableDefinition(table).cfMetaData().values())) {
		...
		try {
			server.registerMBean(this, new ObjectName(
					"org.apache.cassandra.db:type=Table,name=" + table));
		} catch (Exception e) {
	...
	public void createReplicationStrategy(KSMetaData ksm)
		...
		if (replicationStrategy != null)
			StorageService.instance.getTokenMetadata().unregister(
					replicationStrategy);

		replicationStrategy = AbstractReplicationStrategy
				.createReplicationStrategy(ksm.name, ksm.strategyClass,
	...
	public void dropCf(Integer cfId) throws IOException {
		...
		if (cfs.isXMLDomCf()) {
			int colID = ColumnFamilyStore.getColID(cfs.columnFamily);
			XMLDBCatalogManager.instance().removeCollectionInCache(colID);
			XMLDBStore.instance().removeCollectionHandler(colID);
		}
	}
	...
	public void initCf(Integer cfId, String cfName) {
		assert !columnFamilyStores.containsKey(cfId) : String.format(
				"tried to init %s as %s, but already used by %s", cfName, cfId,
				columnFamilyStores.get(cfId));
	...
	public Row getRow(QueryFilter filter) throws IOException {
		ColumnFamilyStore cfStore = getColumnFamilyStore(filter
				.getColumnFamilyName());
	...
	public void apply(RowMutation mutation, boolean writeCommitLog)
			throws IOException {
		/**
		 * modified by xiafan make this function able to update the xml index
		 */
		if (logger.isDebugEnabled())
		...
		try {
			...
			DecoratedKey<?> key = StorageService.getPartitioner().decorateKey(
					mutation.key());
			for (ColumnFamily cf : mutation.getColumnFamilies()) {
				...
				if (cfs.isXMLDomCf()) {
					if (cf.isMarkedForDelete()) {
						// here we means delete the whole xml documents
						oldIndexedColumns = cfs.getColumnFamily(key,
								new QueryPath(cfs.columnFamily),
								ByteBufferUtil.EMPTY_BYTE_BUFFER,
								ByteBufferUtil.EMPTY_BYTE_BUFFER, false,
								Integer.MAX_VALUE);
					} else {
						mutatedIndexedColumns = cf.getColumnNames();
					}
				} else {
					for (ByteBuffer column : cfs.indexManager
							.getIndexedColumns()) {
						if (cf.getColumnNames().contains(column)
							...
							if (logger.isDebugEnabled()) {
								...
								logger.debug(String.format(
										"mutating indexed column %s value %s",
										cf.getComparator().getString(column),
										value == null ? "null" : ByteBufferUtil
												.bytesToHex(value)));
							}
						}
					}
				}
				prepareApplyLatencyTracker.addMicro(System.currentTimeMillis()
						- applyStart);
				applyStart = System.currentTimeMillis();
				synchronized (indexLockFor(mutation.key())) {
					if (mutatedIndexedColumns != null) {
						// with the raw data CF, we can just apply every update
						// in any order and let
						...
						// for obsolete writes.
						long startReadIdxCol = System.currentTimeMillis();
						if (oldIndexedColumns == null)
							oldIndexedColumns = readCurrentIndexedColumns(key,
									cfs, mutatedIndexedColumns);
						readIdxColLatencyTracker.addMicro(System
								.currentTimeMillis() - startReadIdxCol);
					...
					long cfsApplyStart = System.currentTimeMillis();
					cfs.apply(key, cf);
					cfsApplyLatencyTracker.addMicro(System.currentTimeMillis()
							- cfsApplyStart);

					if (mutatedIndexedColumns != null
							|| oldIndexedColumns != null) {
						// ignore full index memtables -- we flush those when
						// the "master" one is full
						long updateIdxStart = System.currentTimeMillis();
						cfs.indexManager.applyIndexUpdates(mutation.key(), cf,
								mutatedIndexedColumns, oldIndexedColumns);
						updateIdxLatencyTracker.addMicro(System
								.currentTimeMillis() - updateIdxStart);
					}
				}
				applyPartLatencyTracker.addMicro(System.currentTimeMillis()
						- applyStart);
			}
		} finally {
			switchLock.readLock().unlock();
		}
		applyLatencyTracker.addMicro(System.currentTimeMillis() - start);
	}
	...
	private static void ignoreObsoleteMutations(ColumnFamily cf,
		...
		// https://issues.apache.org/jira/browse/CASSANDRA-2604)

		if (oldIndexedColumns == null)
			return;

		for (Iterator<ByteBuffer> iter = mutatedIndexedColumns.iterator(); iter
				.hasNext();) {
			ByteBuffer name = iter.next();
			IColumn newColumn = cf.getColumn(name); // null == row delete or it
													// wouldn't be marked
													// Mutated
			if (newColumn != null && cf.isMarkedForDelete()) {
				...
				// the row tombstone, treat it as if it didn't exist. Otherwise
				// we don't care about row
			...
			// deletions are irrelevant to the index unless we're changing state
			// from live -> deleted, i.e.,
			// just updating w/ a newer tombstone doesn't matter
			boolean bothDeleted = (newColumn == null || newColumn
					.isMarkedForDelete())
			...
			boolean obsoleteRowTombstone = newColumn == null
					&& oldColumn != null
					&& cf.getMarkedForDeleteAt() < oldColumn.timestamp();
			boolean obsoleteColumn = newColumn != null
					&& (newColumn.timestamp() <= oldIndexedColumns
							.getMarkedForDeleteAt() || (oldColumn != null && oldColumn
							.reconcile(newColumn) == oldColumn));
			if (bothDeleted || obsoleteRowTombstone || obsoleteColumn) {
				if (logger.isDebugEnabled())
					logger.debug("skipping index update for obsolete mutation of "
							+ cf.getComparator().getString(name));
				iter.remove();
	...
	private static ColumnFamily readCurrentIndexedColumns(DecoratedKey<?> key,
			ColumnFamilyStore cfs, SortedSet<ByteBuffer> mutatedIndexedColumns) {
		QueryFilter filter = QueryFilter
				.getNamesFilter(key, new QueryPath(cfs.getColumnFamilyName()),
			...
			synchronized (cfs.table.indexLockFor(key.key)) {
				...
				if (cfs.isXMLDomCf()) {
					QueryFilter filter = QueryFilter.getSliceFilter(key,
							new QueryPath(cfs.getColumnFamilyName()),
							ByteBufferUtil.EMPTY_BYTE_BUFFER,
							ByteBufferUtil.EMPTY_BYTE_BUFFER, false, 100000000);
					cf = cfs.getColumnFamily(filter);
				} else
					cf = readCurrentIndexedColumns(key, cfs, indexedColumns);
				if (cf != null)
					try {
						cfs.indexManager.applyIndexUpdates(key.key, cf,
	...
	public String getDataFileLocation(long expectedSize) {
		String path = DatabaseDescriptor.getDataFileLocationForTable(name,
				expectedSize);
		// Requesting GC has a chance to free space only if we're using mmap and
		// a non SUN jvm
		if (path == null
				&& (DatabaseDescriptor.getDiskAccessMode() == Config.DiskAccessMode.mmap || DatabaseDescriptor
						.getIndexAccessMode() == Config.DiskAccessMode.mmap)
				&& !MmappedSegmentedFile.isCleanerAvailable()) {
			StorageService.instance.requestGC();
			// retry after GCing has forced unmap of compacted SSTables so they
			// can be deleted
			// Note: GCInspector will do this already, but only sun JVM supports
			// GCInspector so far
	...
	public long getReadIdxColLatency() {
		return readIdxColLatencyTracker.getTotalLatencyMicros();
	}

	@Override
	public double getAvgReadIdxColLatency() {
		long n = readIdxColLatencyTracker.getOpCount();
		long latency = readIdxColLatencyTracker.getTotalLatencyMicros();
		if (n == 0)
			return 0;
	...
	public long getApplyLatency() {
		return applyLatencyTracker.getTotalLatencyMicros();
	}

	@Override
	public double getAvgApplyLatency() {
		long n = applyLatencyTracker.getOpCount();
		long latency = applyLatencyTracker.getTotalLatencyMicros();
		if (n == 0)
			return 0;
		return (float) latency / (float) n;
	}
	...
	public double getAvgUpdateIdxLatency() {
		long n = updateIdxLatencyTracker.getOpCount();
		long latency = updateIdxLatencyTracker.getTotalLatencyMicros();
		if (n == 0)
			return 0;
		return (float) latency / (float) n;
	}
	...
	public double getAvgCfsApplyLatency() {
		long n = cfsApplyLatencyTracker.getOpCount();
		long latency = cfsApplyLatencyTracker.getTotalLatencyMicros();
		if (n == 0)
			return 0;
		return (float) latency / (float) n;
	}
	...
	public long[] getApplyTimeSlice() {
		long[] ret = new long[] { applyLatencyTracker.getTotalLatencyMicros(),
				readIdxColLatencyTracker.getTotalLatencyMicros(),
				cfsApplyLatencyTracker.getTotalLatencyMicros(),
				updateIdxLatencyTracker.getTotalLatencyMicros() };
		return ret;
	}

	@Override
	public long[] getApplyTime() {
		return new long[] { prepareApplyLatencyTracker.getTotalLatencyMicros(),
				applyPartLatencyTracker.getTotalLatencyMicros() };
	}
}
