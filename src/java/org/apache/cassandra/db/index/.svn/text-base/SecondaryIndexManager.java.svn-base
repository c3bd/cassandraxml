...
/**
 * Manages all the indexes associated with a given CFS Different types of
 * indexes can be created across the same CF
 */
public class SecondaryIndexManager {
	private static final Logger logger = LoggerFactory
			.getLogger(SecondaryIndexManager.class);
	 ...
	 * index type has isRowLevelIndex() == true
	 * 
	...
	private XMLIndexManager xmlIndexManager;
	...
	public SecondaryIndexManager(ColumnFamilyStore baseCfs) {
		...
		if (baseCfs.isXMLDomCf()) {
			/*
			 * XMLNodeIndex xmlIndex = new XMLNodeIndex(); xmlIndex.baseCfs =
			 * baseCfs; xmlIndex.init(); xmlIndex.buildIndexAsync();
			 * rowLevelIndexMap.put(XMLNodeIndex.class, xmlIndex);
			 */
			xmlIndexManager = new XMLIndexManager(baseCfs);
		}
	}
	...
	public void reload() throws IOException {
		...
		for (ByteBuffer indexedColumn : indexedColumnNames) {
			ColumnDefinition def = baseCfs.metadata.getColumn_metadata().get(
					indexedColumn);
			if (def == null || def.getIndexType() == null)
				removeIndexedColumn(indexedColumn);
		}

		for (ColumnDefinition cdef : baseCfs.metadata.getColumn_metadata()
				.values())
			if (cdef.getIndexType() != null
					&& !indexedColumnNames.contains(cdef.name))
				addIndexedColumn(cdef);

		if (xmlIndexManager != null)
			xmlIndexManager.reload();
	}

	/**
	 * Does a full rebuild of the indexes specified by columns from the
	 * sstables. Does nothing if columns is empty.
	 * 
	 * @param sstables
	 *            the data to build from
	 * @param columns
	 *            the list of columns to index
	 * @throws IOException
	 */
	public void maybeBuildSecondaryIndexes(Collection<SSTableReader> sstables,
			SortedSet<ByteBuffer> columns) throws IOException {
		if (columns.isEmpty())
			return;

		logger.info(String.format(
				"Submitting index build of %s for data in %s",
				baseCfs.metadata.comparator.getString(columns),
		...
		logger.info("Index build of "
				+ baseCfs.metadata.comparator.getString(columns) + " complete");
	}
	 ...
	 * @return a future which the caller can optionally block on signaling the
	 *         index is built
	 */
	public synchronized Future<?> addIndexedColumn(ColumnDefinition cdef) {
		...
		if (index instanceof PerRowSecondaryIndex) {
			SecondaryIndex currentIndex = rowLevelIndexMap
					.get(index.getClass());
	...
	public SecondaryIndex getIndexForColumn(ByteBuffer column) {
		if (rowLevelIndexMap.isEmpty())
			return indexesByColumn.get(column);
		else
			return rowLevelIndexMap.values().iterator().next();
	}
	...
	public void unregisterMBeans() {
		for (Map.Entry<ByteBuffer, SecondaryIndex> entry : indexesByColumn
				.entrySet())
	...
	public void removeAllIndexes() throws IOException {
		if (xmlIndexManager != null)
			xmlIndexManager.removeAllIndexes();
		for (Map.Entry<ByteBuffer, SecondaryIndex> entry : indexesByColumn
				.entrySet())
	...
	public void renameIndexes(String newCfName) throws IOException {
		if (xmlIndexManager != null)
			xmlIndexManager.renameIndexes(newCfName);
	 ...
	 */
	public DecoratedKey<LocalToken> getIndexKeyFor(ByteBuffer name,
			ByteBuffer value) {
		return new DecoratedKey<LocalToken>(new LocalToken(baseCfs.metadata
				.getColumnDefinition(name).getValidator(), value), value);
	}
	...
	public Collection<ColumnFamilyStore> getIndexesBackedByCfs() {
		...
		for (SecondaryIndex index : rowLevelIndexMap.values()) {
			ColumnFamilyStore cfs = index.getUnderlyingCfs();

			if (cfs != null)
				cfsList.add(cfs);
		}
		if (xmlIndexManager != null)
			cfsList.addAll(xmlIndexManager.getIndexesBackedByCfs());
		return cfsList;
	}
	 ...
	 * and mutated columns.
	 * 
	 * For columns whos underlying index type has the isRowLevelIndex() flag set
	 * to true this function will call the
	 * 
	 * @param rowKey
	 *            the row key
	 * @param cf
	 *            the current rows data
	 * @param mutatedIndexedColumns
	 *            the set of columns that were changed or added
	 * @param oldIndexedColumns
	...
	public void applyIndexUpdates(ByteBuffer rowKey, ColumnFamily cf,
		...
		if (xmlIndexManager != null) {
			xmlIndexManager.applyIndexUpdates(rowKey, cf,
			...
			for (ByteBuffer columnName : oldIndexedColumns.getColumnNames()) {
				...
				if (index instanceof PerRowSecondaryIndex) {
					...
					if (appliedRowLevelIndexes.add(index.getClass()))
						((PerRowSecondaryIndex) index).applyIndexUpdates(
								rowKey, cf, mutatedIndexedColumns,
								oldIndexedColumns);

					/*
					 * modified by xiafan(xiafan68@gmail.com) there is no need
					 * to loop any more?
					 */
					break;
				} else {
					DecoratedKey<LocalToken> valueKey = getIndexKeyFor(
		...
		if (mutatedIndexedColumns != null) {
			...
			for (ByteBuffer columnName : mutatedIndexedColumns) {
				...
				if (index instanceof PerRowSecondaryIndex) {
					...
					((PerColumnSecondaryIndex) index).insertColumn(valueKey,
							rowKey, column);
				}
			}
		}
	...
	public void deleteFromIndexes(DecoratedKey<?> key,
			...
			throws IOException {
		if (xmlIndexManager != null)
			xmlIndexManager.deleteFromIndex(key, indexedColumnsInRow, allColumns);
		...
		for (IColumn column : indexedColumnsInRow) {
			...
			if (index instanceof PerRowSecondaryIndex) {
				...
				else if (cleanedRowLevelIndexes.add(index.getClass()))
					((PerRowSecondaryIndex) index).deleteFromIndex(key,
							indexedColumnsInRow, allColumns);
			} else {
				DecoratedKey<LocalToken> valueKey = getIndexKeyFor(
	 ...
	 */
	private List<SecondaryIndexSearcher> getIndexSearchersForQuery(
			IndexClause clause) {
		...
		for (IndexExpression ix : clause.expressions) {
			...
			Set<ByteBuffer> columns = groupByIndexType.get(index.getClass()
					.getCanonicalName());
		...
		for (Map.Entry<String, Set<ByteBuffer>> entry : groupByIndexType
				.entrySet()) {
			indexSearchers.add(getIndexForColumn(
					entry.getValue().iterator().next())
					.createSecondaryIndexSearcher(entry.getValue()));
		}
	...
	/**
	 * Performs a search across a number of column indexes TODO: add support for
	 * querying across index types
	 * 
	 * @param clause
	 *            the index query clause
	 * @param range
	 *            the row range to restrict to
	 * @param dataFilter
	...
	public List<Row> search(IndexClause clause, AbstractBounds range,
		...
		if (indexSearchers.size() > 1)
			throw new RuntimeException(
					"Unable to search across multiple secondary index types");

		return indexSearchers.get(0).search(clause, range, dataFilter);
	}

	public SecondaryIndex getRowLevelIndex() {
		if (!rowLevelIndexMap.isEmpty())
			return rowLevelIndexMap.values().iterator().next();
		else
			return null;
