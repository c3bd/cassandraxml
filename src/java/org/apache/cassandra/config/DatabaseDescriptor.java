...
public class DatabaseDescriptor
{
    ...
    private static InetAddress xmlRpcAddress;
    ...
    static
    {
        try
        {
           ...
           if (conf.in_memory_compaction_limit_in_mb != null && conf.in_memory_compaction_limit_in_mb <= 0)
            ...
            //modified by xiafan
            if (conf.xml_rpc_address != null) {
            	try {
					xmlRpcAddress = InetAddress.getByName(conf.xml_rpc_address);
				} catch (UnknownHostException e) {
					 throw new ConfigurationException("Unknown xmlrpc address '" + conf.xml_rpc_address + "'");
				}
            }
            ...
            //hardcoded xml system tables
            KSMetaData XMLSystemMeta = new KSMetaData(SysConfig.SYSCATALOG, SimpleStrategy.class, KSMetaData.optsWithRF(conf.sys_keyspace_replica), SysConfig.CollectionCf, SysConfig.XMLCf, SysConfig.XMLSchemaCf,SysConfig.UserCf);
            Schema.instance.load(SysConfig.CollectionCf);
            Schema.instance.load(SysConfig.XMLCf);
            Schema.instance.load(SysConfig.XMLSchemaCf);
            Schema.instance.load(SysConfig.UserCf);
            Schema.instance.addSystemTable(XMLSystemMeta);
           
            //hardcoded xml data keyspace
            CFMetaData rootCol = SysConfig.rootColCf;
            KSMetaData XMLDataKeyspace = new KSMetaData(SysConfig.DEFAULT_KEYSPACE, SysConfig.DEFAULT_KEYSPACE_PARTITIONER, KSMetaData.optsWithRF(conf.data_keyspace_replica), rootCol);
            Schema.instance.load(rootCol);
            Schema.instance.addSystemTable(XMLDataKeyspace);
   ...
   public static String getClusterName()
    ...
    public static InetAddress getXmlRpcAddress() {
    	return xmlRpcAddress;
    }
    
    public static int getXmlRpcPort() {
    	return conf.xml_rpc_port;
    }
    
    public static int getBtreeCacheSize() {
    	return conf.btree_cache_size;
    }
    
    public static int getNodeTagCacheSize() {
    	return conf.nodeTag_cache_size;
    }
    
    public static int getBtreeAdjustInMs() {
    	return conf.btree_perform_adjust_inms;
    }
    
    public static double getAdjustStep() {
    	return conf.adjust_step;
    }
    
    public static Config.XMLServerMode getServerMode() 
    {
    	return conf.xml_server_mode;
    }
    
    public static boolean ZooKeeperEnabled() {
    	return conf.zookeeper_enabled;
    }
    
    public static String[] getZooKeeperServerList() {
    	return conf.zookeeper_server_list;
    }
    
    public static String getZooKeeperDir() {
    	return conf.zookeeper_dir;
    }
    
    public static int getSessionTimeOut(){
    	return conf.session_timeout;
    }
    
    public static int getDomNodeCacheSize() {
    	return conf.domnode_cache_size;
    }
}
