...
public class Config
{
    ...
    public String xml_rpc_address;
    public Integer xml_rpc_port = 8081;
    
    public Integer btree_cache_size = new Integer(1024 * 1024);  //128m
    public Integer nodeTag_cache_size = new Integer(128 * 1024);
    
    public int btree_perform_adjust_inms = 60 * 30 * 1000; //adjust every 30 minutes
    public double adjust_step = 0.1;
    
    //the following two fields together manage the XML cluster server
    public boolean zookeeper_enabled = false;
    public String[] zookeeper_server_list = null;
    public String zookeeper_dir = "/gms/" + cluster_name ;
	public int session_timeout = 1000; 
    public XMLServerMode xml_server_mode = XMLServerMode.slave;
    
    public int domnode_cache_size = 100000;  //the number of dom nodes to be cached for each collection
    public int sys_keyspace_replica = 1;    //define the xml system keyspace replication
    public int data_keyspace_replica = 1;  //define the data keyspace replication
    
    public static enum XMLServerMode {
    	master,				//only master nodes can receive update queries and update the system catalog
    	secondary_master,   //secondary_master functions much like a slave,  except that it will seize role of master 
    	slave,
    	starting,
    	started
    }
