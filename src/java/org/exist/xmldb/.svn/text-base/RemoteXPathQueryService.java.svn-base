...
public class RemoteXPathQueryService implements XPathQueryServiceImpl, XQueryService {
	...
	public ResourceSet query(String query, boolean recursive)
			throws XMLDBException {
    	return query(query, recursive, null);
    }
     ...
     * @exception XMLDBException if an error occurs
     */
    public ResourceSet query(String query, boolean recursive, String sortExpr)
        throws XMLDBException {
        try {
        	HashMap optParams = new HashMap();
        	optParams.put("colPath", collection.getPath());
            if(sortExpr != null)
			...
			params.add(query.getBytes(SysConfig.ENCODING));
